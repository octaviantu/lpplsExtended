...
class CountMetrics:
    ...
    def add_bubble(conditions: dict, t2_index: int):
        ...
        if is_accepted:
            ...
            if t2_index not in CountMetrics.rejected_reasons_per_t2:
                CountMetrics.rejected_reasons_per_t2[t2_index] = {k: 0 for k in CountMetrics.rejected_reasons.keys()}
            for key, value in conditions.items():
                if not value:
    ...
    def print_metrics():
        ...
        with open('./temp_out/metrics_per_turn.csv', 'w', newline='') as csvfile:
            fieldnames = ['t2_index', 'accepted', 'rejected'] + list(CountMetrics.rejected_reasons.keys())
            ...
            for t2_index, reasons in CountMetrics.rejected_reasons_per_t2.items():
                ...
                row = {'t2_index': t2_index, 'accepted': accepted_percentage, 'rejected': rejected_percentage}
