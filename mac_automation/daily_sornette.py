...
with open(daily_all_calls_log, "a") as f:
    f.write(f"{start_time} SCRIPT_STARTED\n")
...
if last_log_entry:
    ...
    if today == last_run_date and (
        last_status == "SUCCESS"
        or (last_status == "RUNNING" and time_difference < timedelta(hours=3))
    ):
        with open(daily_all_calls_log, "a") as f:
            f.write(f"{start_time} SCRIPT_EXITED_EARLY with status {last_status}\n")
        exit(0)
else:
    with open(daily_all_calls_log, "a") as f:
        f.write(f"{start_time} SCRIPT_STARTED\n")
...
with open(execution_output_log, "w") as out_log, open(execution_error_log, "w") as err_log:
    process = subprocess.run(
        [python_interpreter, python_script], stdout=out_log, stderr=err_log, cwd=working_directory
    )
...
with open(daily_run_status_log, "a") as f:
    if exit_code == 0:
        f.write(f"{end_time} SUCCESS\n")
        subprocess.run(
            [
                "osascript",
                "-e",
                'display notification "Sornette finished successfully" with title "Sornette Status"',
            ]
        )
    else:
        f.write(f"{end_time} FAILED\n")

# Log that the script finished
with open(daily_all_calls_log, "a") as f:
    f.write(f"{end_time} SCRIPT_FINISHED_WITH_CODE_{exit_code}\n")
